...
/**
 * Generate a start list for the given comp and date.
 */
module.exports = function startList(
  seriesName, seriesYear, comp, date, lookup, rerandomize, callback) {
  ...
  async.auto({
    ...
    function createStartList(done, results) {
      ...
      function forCompAndDate(cd) {
        return cd.comp === comp && cd.date.substr(0, 10) === date;
      });
      dayDivisions = _.sortBy(dayDivisions, "order");
      ...
      var athleteDivisions = _.map(dayDivisions, function gatherAthletes(dd) {
        ...
        allForDiv = _.sortBy(allForDiv, "date");
        var
          ddIx = 0,
          curDD = allForDiv[ddIx];
        while (curDD && (curDD.date !== dd.date)) {
          ddIx += 1;
          curDD = allForDiv[ddIx];
        }
        ddIx += 1;
