  ...
  async.auto({
    ...
    "validation": [
      ...
      function (results, cb) {
        ...
        _.forEach(results.startList, function validateAthlete(a) {
          ...
          if (!result) {
            ...
            _.forEach(result.scores, function checkJudge(judge, judgeIx) {
              // debug("[debug] scores for bib #%s for judge #%s: %j", a.bib, judgeIx + 1, judge);
              _.forEach(expectedKeys, function checkKey(key) {
                // debug("[debug] score for bib #%s, judge #%s, '%s'=%s, type=%s",
                //   a.bib, judgeIx + 1, key, judge[key], typeof judge[key]);
                if (judge[key] === undefined || judge[key] === null || _.isNumber(judge[key])) {
          ...
          if (clean && (result.score >= relevantScore)) {
            ...
            _.forEach(expectedKeys, function compare(key) {
              var
                min = 99,
                max = 0;
              _.forEach(result.scores, function checkJudge(judge) {
