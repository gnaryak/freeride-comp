...
/* eslint-enable max-params */
  ...
  async.auto({
    ...
    "compsInOverall": function getCompsInOverall(done) {
      util.retrieveFromS3(keyPrefix + "compsInOverall.json",
      function assignCompCount(compCountErr, compsInOverall) {
        var compCount = compCountErr ? 3 : compsInOverall.compsInOverall;
        console.log("compsInOverall=%s", compCount);
        done(null, compCount);
      });
    },
    ...
    "aggregatedResults": ["compResults", "selectedCompetitors",
      function assembleResults(results, done) {
        ...
        _.forEach(results.selectedCompetitors, function aggregateResults(athlete) {
          ...
          for (var ix = 0; ix < results.compsInOverall; ix += 1) {
            if (sortedPoints[ix]) {
